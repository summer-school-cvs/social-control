...
contract Factory {
    ...
    function addMember(address val) public returns (address) {
        ...
        require(
            add_member_candidates[val] != address(0),
            "Already on the list of candidates."
        );
    ...
    function exclude(address val) public onlyMember returns (address) {
        require(
            delete_member_candidates[val] != address(0),
            "Already on the list of candidates for exclusion."
        );
    ...
    function leave() public onlyMember returns (address) {
        require(
            delete_member_candidates[msg.sender] != address(0),
            "Already on the list of candidates for exclusion."
        );
    ...
    function isMember(address val) public view returns (bool) {
    ...
    function votePower(address) public onlyMember returns (uint256) {
        return 1;
    }
